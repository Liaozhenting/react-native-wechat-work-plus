...
export const shareLinkAttachment = (data) => {
  return new Promise((resolve, reject) => {
    data.webpageUrl = encodeURI(data.webpageUrl)
    WeChatWork.shareLinkAttachment(data);
    DeviceEventEmitter.once('Share.Resp', resp => {
      if (resp.errCode === 0) {
        resolve(resp);
      } else {
...
export const shareImage = (data) => {
  return new Promise((resolve, reject) => {
    ...
    WeChatWork.shareLocalImage(data);
    DeviceEventEmitter.once('Share.Resp', resp => {
      if (resp.errCode === 0) {
        resolve(resp);
      } else {
